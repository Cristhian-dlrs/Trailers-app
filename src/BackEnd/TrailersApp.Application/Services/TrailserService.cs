...
namespace TrailersApp.Application.Services
{
    public class TrailserService : ITrailserService
    {
        ...
        public async Task<IEnumerable<TrailersListDto>> GetAllTrailers(string filter = null)
        {
            var trailers = await _trailerRepo.GetAll();
            if (filter != null)
                trailers = trailers.Where(x => x.Name.ToLower().Contains(filter.ToLower()));

            var result = _mapper.Map<IEnumerable<TrailersListDto>>(trailers);
        ...
        public async Task UpdateTrailer(UpdateTrailerDto updateTrailerDto)
        {
            ...
            trailer.CoverSrc = _image.GetImgSrc(updateTrailerDto.Image, "trailers");
